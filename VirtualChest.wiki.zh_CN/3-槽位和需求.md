...
`PrimaryShiftAction`和`SecondaryShiftAction`同`PrimaryAction`与`SecondaryAction`十分相似，但是他们只有在玩家按住Shift按键的时候点击菜单才会激活，如果您没有设置这一项的话， `PrimaryShiftAction` 的执行效果将会等同于 `PrimaryAction`，同样的，`SecondaryShiftAction` 也会在同样情况下执行同 `SecondaryAction` 一样的行为。
